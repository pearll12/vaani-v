...
export async function assignDeliveryAgent(orderId) {
  try {
    ...
    // In this app, business_id is usually synonymous with profile_id (the auth.uid() that created it)
    // We'll need to fetch the profile first to get the ID, or assume it's the one that owns the order.
    // For now, we'll fetch the first profile to match current multi-tenant logic (one business app)
    const { data: profiles } = await supabase.from('business_profiles').select('id').limit(1const { data: profiles } = await supabase
    ...
    if (!profileId) {
      console.error('❌ No business profile found for assignment')
      return null
    }

    const agent = await getNextDeliveryAgent(profileId)
    if (!agent) {
      console.log('⚠️ No delivery agents configured for this business. Skipping assignment.')
      return null
    }
