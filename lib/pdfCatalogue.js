...
export async function generateAndUploadCataloguePDF(catalogMap) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Group by category from catalogMap
      const categories = {}
      ...
      for (const [cat, items] of Object.entries(categories)) {
        ...
        items.forEach((entry, i) => {
          ...
          const itemLabel = brand ? `${entry.idx}. ${entry.item.name} (${brand})` : `${entry.idx}. ${entry.item.name}`

          doc.text(itemLabel, 50, doc.y + 5)
          doc.text(`₹${price} / ${entry.item.unit}`, doc.page.width - 150, doc.y + 5, { width: 100, align: 'right' })
          doc.y += 20
        })
