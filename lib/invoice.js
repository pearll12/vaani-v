...
export async function generateAndSendInvoice(orderId, phone) {
  try {
    ...
    doc.rect(summaryX, doc.y, summaryWidth, 100).fillAndStroke('#f8fafc', '#e2e8f0')
    
    doc.fillColor('#475569').fontSize(10)
    doc.text('Subtotal:', summaryX + 20, doc.y + 15)
    doc.text(`₹${Number(subtotal).toFixed(2)}`, summaryX + 100, doc.y + 15, { align: 'right', width: 90 })
    
    doc.text('CGST (9%):', summaryX + 20, doc.y + 35)
    doc.text(`₹${Number(cgst).toFixed(2)}`, summaryX + 100, doc.y + 35, { align: 'right', width: 90 })
    
    doc.text('SGST (9%):', summaryX + 20, doc.y + 55)
    doc.text(`₹${Number(sgst).toFixed(2)}`, summaryX + 100, doc.y + 55, { align: 'right', width: 90 })
    
    doc.rect(summaryX + 15, doc.y + 75, summaryWidth - 30, 1).fill('#e2e8f0') // Divider

    doc.fillColor('#1e293b').fontSize(12).font(hasFonts ? fontPath : 'Helvetica-Bold')
    doc.text('Grand Total:', summaryX + 20, doc.y + 82)
    doc.fillColor('#00e5c3').text(`₹${Number(grandTotal).toFixed(2)}`, summaryX + 100, doc.y + 82, { align: 'right', width: 90 })
    ...
    if (paymentLink) {
      ...
      doc.y += 30
      doc.rect(50, doc.y, doc.page.width - 100, 45).fill('#f0fdf4') // Light green bg
      ...
      doc.text(`Click here to pay or use UI below: ${paymentLink}`, 70, doc.y + 25, {
        link: paymentLink,
        underline: true
      })
    }
